"""
Transform train.json to research SFT format:
- classify_error: short "Primary: Subtype" (no period), 5 allowed primaries.
- explain_error: "The compiler reports: ... Category: ... Reason: ... Fix: ..."
"""
import json
import re
...
PRIMARIES = ("Syntax Error", "Semantic Error", "Linker Error", "Type Error", "Preprocessor Error")

# Map current targets to normalized Primary: Subtype (and to Linker/Preprocessor where needed)
CLASSIFY_NORMALIZE = {
    "Semantic Error: Undefined External Reference": "Linker Error: Undefined Reference",
    "Semantic Error: Static Function Without Definition": "Linker Error: Missing Definition",
    "Syntax Error: Unbalanced Preprocessor Directive": "Preprocessor Error: Unbalanced Directive",
    "Syntax Error: Incomplete Include Directive": "Preprocessor Error: Incomplete Include",
    "Syntax Error: Missing Header": "Preprocessor Error: Missing Header",
    "Syntax Error: Invalid Macro": "Preprocessor Error: Invalid Macro",
    "Syntax Error: Macro Expansion Error": "Preprocessor Error: Macro Expansion Error",
}
# Subtype name standardizations (optional)
CLASSIFY_SUBTYPE_ALIAS = {
    "Semantic Error: Conflicting Linkage": "Semantic Error: Conflicting Linkage Specifiers",
    "Semantic Error: Inline Redefinition Violation": "Semantic Error: One Definition Rule Violation",
}


def normalize_classify_target(target: str) -> str:
    """Return short 'Primary: Subtype' with no trailing period. Use only allowed primaries."""
    t = target.strip().rstrip(".")
    t = CLASSIFY_NORMALIZE.get(t, t)
    t = CLASSIFY_SUBTYPE_ALIAS.get(t, t)
    return t


def parse_explain_input(inp: str) -> Tuple[str, str]:
    """Extract code and full compiler output from explain_error input."""
...
def first_compiler_line(compiler_output: str) -> str:
    """Exact first line of compiler message for 'The compiler reports: ...'."""
    return compiler_output.split("\n")[0].strip() if compiler_output else ""


def category_for_explain(primary_subtype: str) -> str:
    """Category line: 'Primary – Subtype' (en-dash)."""
    return primary_subtype.replace(": ", " – ", 1)


def _q(s: str) -> str:
    """Quote identifier from message."""
    m = re.search(r"['\u2018\u2019]([^'\u2018\u2019]+)['\u2018\u2019]", s)
    return m.group(1).strip() if m else ""


def generate_reason_fix(code: str, compiler_output: str) -> Tuple[str, str]:
    """Return (reason, fix) with concrete technical explanation and steps. No vague phrases."""
    msg = compiler_output
    first = msg.split("\n")[0] if msg else ""
    ...
    if "narrowing conversion" in first or ("narrowing" in first and "{" in code):
        return (
    ...
    if "expected" in first and ";" in first:
        ...
        if "before ')'" in first:
            return (
                "The parser expected a closing parenthesis before the semicolon. Parentheses must be balanced.",
                "Add the missing ')' or remove the misplaced semicolon so the expression is correctly parenthesized."
            )
        return (
    ...
    if "expected ')' at end" in first or "')' at end of input" in first:
        return (
            "The compiler reached end of input while expecting a closing parenthesis. This often occurs when a macro or expression is missing a closing ')'.",
            "Add the missing closing parenthesis in the macro definition or the expression."
        )

    # Undeclared / scope
    if "was not declared" in first or "were not declared" in first:
        name = _q(first)
        if name or " did you mean " in first:
            return (
                f"The identifier '{name}' is used but has not been declared in this scope. In C++, every identifier must be declared before use.",
                "Declare the variable or function before use, or fix the spelling (e.g. use the compiler-suggested alternative)."
            )
        return (
            "An identifier is used in a scope where it is not visible. In C++, variables and functions must be declared before use and are visible only within their scope.",
            "Move the declaration above the use, or ensure the name is in scope (correct block or namespace)."
        )
    ...
    if "return-statement with no value" in first and "returning 'int'" in first:
        return (
            "A return statement has no value in a function declared to return a type (e.g. int). Every return path in a non-void function must supply a value of the return type.",
    ...
    if "expected ')' at end of input" in first and "#define" in code:
        return (
            "The macro is defined with unbalanced parentheses. When expanded, the parser reaches end of input expecting a closing parenthesis.",
    ...
    if "expected ';' before ')'" in first and "for" in code:
        return (
            "The for-loop header is missing a semicolon. The form is for (init; condition; increment) with two semicolons.",
            "Add the missing semicolon between the condition and the increment (or between init and condition)."
        )
    if "'else' without a previous 'if'" in first:
        return (
            "An else clause appears without a matching if. In C++, else must immediately follow an if (or else if) block.",
    ...
    if "cannot convert" in first and "'int*'" in first and "'double*'" in first:
        return (
            "A pointer to one type is assigned to a pointer to an unrelated type. C++ does not allow implicit conversion between pointers to different object types.",
    ...
    if "'void*' is not a pointer-to-object" in first or "dereferencing void" in first:
        return (
            "A void pointer is dereferenced. The type and size of the object pointed to by void* are unknown, so dereference is not allowed.",
    ...
    if "'free' called on pointer allocated with 'new'" in first or "allocated with 'new'" in first:
        return (
            "Memory allocated with C++ new is being freed with C free. C++ and C allocation must not be mixed.",
            "Use delete (or delete[]) for memory from new."
        )

    # Class / access (duplicate for "private within")
    if "is private within this context" in first or "is protected within this context" in first:
        return (
            "A member is accessed from outside its allowed context. Private and protected members are not accessible from arbitrary code.",
            "Use a public member or interface, or add a friend declaration or accessor."
        )

    # Smart pointers / double free
    if "raw pointer owned by 'shared_ptr'" in first or "'delete' called on raw pointer" in first:
        return (
            "A raw pointer obtained from a smart pointer (e.g. get()) is passed to delete. The smart pointer already owns the object and will delete it; manual delete causes double free.",
    ...
    if "declaration of 'auto" in first and "has no initializer" in first:
        return (
            "A variable declared with auto has no initializer. The type of auto is deduced from its initializer, so one must be provided.",
    ...
    if "cannot convert 'const char*' to 'int' in assignment" in first and "iterator" in msg.lower():
        return (
            "A value of the wrong type is assigned through an iterator (e.g. to a vector<int> element). The iterator's element type is fixed by the container.",
    ...
    if "no match for 'operator<'" in first and "Point" in first:
        return (
            "std::sort requires operator< for the element type, but it is not defined for the type used.",
    ...
    if "no match for 'operator+'" in first and "int" in first and "std::string" in first:
        return (
            "std::accumulate is used with an initial value and element types that cannot be combined with operator+. The accumulator and element types must support the binary operation.",
            "Use an initial value of the same type as the elements, or use the four-argument form of accumulate with a custom operation."
        )
    if "no match for 'operator-'" in first and "List_iterator" in first:
        return (
            "std::sort requires random-access iterators. std::list provides bidirectional iterators, which do not support operator-.",
    ...
    if "assignment of read-only location" in first and "set" in first:
        return (
            "An attempt is made to modify an element of a set through an iterator. Set elements are const to preserve the ordering invariant.",
    ...
    if "conversion from 'int' to non-scalar type" in first and "iterator" in first:
        return (
            "An iterator is initialized with an integer. Iterators are obtained from containers (e.g. v.begin()) or valid past-the-end values.",
    ...
    if "has both" in first and "static" in first and "extern" in first and "linkage" in first:
        return (
    ...
    if "redefinition of 'void f()'" in first and "inline" in code:
        return (
            "The function f() is defined more than once in a way that violates inline rules. Each function must have a single definition unless it is inline (with the same definition in every translation unit).",
    ...
    if "has both 'extern' and initializer" in first:
        return (
            "A variable is declared extern and given an initializer in the same declaration. extern declarations typically do not define.",
    ...
    if "typeid" in code and "lvalue required" in first:
        return (
            "The result of typeid is used on the left side of an assignment. typeid returns a std::type_info object that is not assignable.",
    ...
    if "cannot convert" in first and "Color" in first and "int" in first:
        return (
            "A scoped enum (enum class) value is used where an int is expected. Scoped enums do not implicitly convert to integers.",
    ...
    if "no match for 'operator=='" in first and "Status" in first:
        return (
            "A scoped enum is compared directly to an integer. Scoped enums do not implicitly convert to int.",
    ...
    if "was not declared in this scope" in first and "did you mean" in first and "::" in first:
        return (
            "A scoped enum enumerator is used without its enum name. With enum class, enumerators must be qualified: EnumName::Enumerator.",
            "Use the full name (e.g. Traffic::Green) where you used the unqualified name."
        )
    ...
    if "has not been declared" in first and "Http" in first:
        return (
            "A nested namespace name is used without the full path. Nested namespaces must be qualified from the enclosing namespace or the global scope.",
    ...
    if "LongProjectName" in first and "has not been declared" in first:
        return (
            "A namespace alias refers to a namespace that has not been declared. The right-hand side of a namespace alias must be an existing namespace.",
            "Declare or include the namespace before using it in an alias."
        )
    if "redefinition of 'int {anonymous}::x'" in first:
        return (
            "The same name is defined in more than one anonymous namespace. Each anonymous namespace is distinct; the same name in the same TU cannot be redefined.",
    ...
    if "inaccessible base of 'Derived'" in first:
        return (
            "A catch clause catches by reference to a base that is an inaccessible base of the thrown type. When catching by reference, the catch parameter type must be accessible.",
    ...
    if "throw' expression with no operand" in first:
        return (
            "A bare throw; is used outside a catch handler. throw; without an operand is only valid inside a catch block to rethrow the current exception.",
    ...
    if "expected '{' before ';' token" in first and "catch" in code:
        return (
            "A catch block has no body. After catch (type) there must be a compound statement in braces.",
            "Add a block in braces after the catch parameter (e.g. catch (...) { })."
        )
    if "expected primary-expression before 'catch'" in first:
        return (
            "A catch block appears without a preceding try block. Each catch must be immediately after a try block.",
    ...
    if "noexcept" in first and "terminate" in first:
        return (
            "An exception is thrown from a function declared noexcept. A noexcept function must not throw; if it does, std::terminate is called.",
    ...
    if "variable length array" in first and "forbids" in first:
        return (
            "A variable-length array (VLA) is used. Standard C++ does not allow array sizes that are not constant expressions.",
    ...
    if "constraints not satisfied" in first or ("constraint" in first and "not satisfied" in msg):
        return (
            "A template or function call does not satisfy its required constraints (e.g. std::integral). Concepts restrict which types can be used with a template.",
            "Pass a type that satisfies the concept, or relax the constraint if the use is valid."
        )
    if "deduced conflicting types" in first:
        return (
            "Template argument deduction produces conflicting types for the same parameter. Each template parameter must deduce to a single type across all arguments.",
    ...
    if "assignment of read-only variable in lambda" in first:
        return (
            "A lambda tries to modify a variable captured by value. By default, captures by value are const inside the lambda.",
    ...
    if "'this' was not captured" in first and "lambda" in first:
        return (
            "A member function or member is used inside a lambda without capturing this. To use the current object or its members, the lambda must capture this.",
            "Add this to the capture list: [this] or [=]."
        )
    if "invalid operands" in first and "operator+" in first and "lambda" in code:
        return (
            "A generic lambda is instantiated with argument types that do not support the operation used. The body uses an operation (e.g. +) that is not valid for the deduced types.",
    ...
    if "no viable conversion from 'lambda'" in first and "std::function" in first:
        return (
            "A lambda cannot be converted to the expected std::function type. The lambda's parameters or return type do not match the target signature.",
            "Change the lambda signature to match the std::function, or use a different target type."
        )

    # More patterns: smart ptr, shift, enum, etc.
    if "dereferencing a released unique_ptr" in first:
        return (
            "A unique_ptr is dereferenced after release() has been called. After release(), the pointer is no longer managed and may be invalid.",
    ...
    if "no match for 'operator|'" in first and "Mode" in first:
        return (
            "A bitwise operator is used on a scoped enum. Scoped enums do not support bitwise operations by default.",
    ...
    if "expected ';' before '~'" in first:
        return (
            "The bitwise NOT operator ~ is used with invalid syntax (e.g. space or typo). The correct form is ~expr.",
    ...
    if "redefinition of 'Red'" in first and "enum" in code:
        return (
            "The same enumerator name appears in more than one unscoped enum in the same scope. Unscoped enums put enumerators in the surrounding scope, so names must be unique.",
    ...
    # Fallback: concrete reason and fix
    return (
...
def build_explain_target(compiler_first_line: str, category_normalized: str, reason: str, fix: str) -> str:
    """Build the exact structured explain target."""
    category_line = category_for_explain(category_normalized)
    return (
        f"The compiler reports: {compiler_first_line}.\n\n"
...
def transform_train(path: Path) -> None:
    """Load, transform classify and explain targets, write back valid JSON."""
    data = json.loads(path.read_text(encoding="utf-8"))
    prev_classify_target: str = ""
    for i, entry in enumerate(data):
        inp = entry.get("input", "")
        if inp.startswith("classify_error:"):
            raw = entry.get("target", "").strip().rstrip(".")
            entry["target"] = normalize_classify_target(raw or entry.get("target", ""))
            prev_classify_target = entry["target"]
        elif inp.startswith("explain_error:"):
            code, compiler_output = parse_explain_input(inp)
            first_line = first_compiler_line(compiler_output)
            reason, fix = generate_reason_fix(code, compiler_output)
            category = prev_classify_target or "Semantic Error: Other"
            entry["target"] = build_explain_target(first_line, category, reason, fix)
        else:
            pass
    path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    n_classify = sum(1 for e in data if e.get("input", "").startswith("classify_error:"))
    n_explain = sum(1 for e in data if e.get("input", "").startswith("explain_error:"))
    print(f"Transformed {n_classify} classify_error and {n_explain} explain_error entries. Written to {path}")


def main() -> None:
    repo = Path(__file__).resolve().parent.parent
    train_path = repo / "data" / "processed" / "train.json"
    transform_train(train_path)
